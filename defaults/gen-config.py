...
COMMON_CONFIG = {
    ...
    "qm_guest_exec": {
        "vmid": "VMID_HERE_CHANGEME",
        "guest_shell": "bash",
        "remote_script": "/root/temperature.sh",
    },
    "truenas_ssh": {
        "host": "truenas",
        "user": "root",
...
PROFILE_OVERRIDES = {
    "Proxmox": {
        "system_info": {
            ...
            "single_zone": False,
            "temp_focus": "Both",
            "disks": ["SERIAL_A", "SERIAL_B", "SERIAL_C", "SERIAL_D"],
        },
        "disk_temps": {
            "method": "ssh",
            "disk_identifier": "serial",
        },
        "fan_curve": {
            "cpu": [
                [0, 35],
...
def main():
    ...
    write_table(lines, "disk_temps", config_object["disk_temps"])
    write_table(lines, "qm_guest_exec", config_object["qm_guest_exec"])
